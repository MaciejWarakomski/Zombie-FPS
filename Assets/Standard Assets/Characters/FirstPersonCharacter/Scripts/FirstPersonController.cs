...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class FirstPersonController : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            if (m_Jump && CheckJumpAngle())
            {
                previousMove = new Vector3(0, 0, 0);
            }
            if (m_Jumping || m_Jump)
            {
                if (Math.Abs(previousMove.x) > 0)
                {
                    m_MoveDir.x = previousMove.x;
                }
                else
                {
                    m_MoveDir.x = 0;
                }
                if (Math.Abs(previousMove.z) > 0)
                {
                    m_MoveDir.z = previousMove.z;
                }
                else
                {
                    m_MoveDir.z = 0;
            ...
            previousMove = m_CharacterController.velocity;
